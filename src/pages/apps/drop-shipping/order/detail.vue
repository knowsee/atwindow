...
function goCopy() {
  router.push({ name: 'apps-drop-shipping-order-create', query: { id: orderId.value, mode: 'copy' } })
}
...
function resolveErpSource(type) {
  ...
  return sourceMap[Number(type)] ? t(sourceMap[Number(type)]) : t('pages.dropShippingOrderDetail.erp.thirdParty')
}
...
async function loadDetail() {
  if (!orderId.value) {
    toast(t('pages.dropShippingOrderDetail.messages.missingId'), 'error')
    loading.value = false
  ...
  try {
    const [co, res] = await Promise.all([
      loadCountryOptions(),
      $api('/ordernew/orderDfDetail', { method: 'GET', query: { id: orderId.value, biz_type: 'df' } }),
    ])
    ...
    if (Number(res?.code) === 1 && res?.data) {
      ...
      toast(res?.msg || t('pages.dropShippingOrderDetail.messages.loadFailed'), 'error')
    }
  }
  catch (e) {
    toast(e?.data?.msg || e?.message || t('pages.dropShippingOrderDetail.messages.loadGenericFailed'), 'error')
  }
  finally {
    loading.value = false
  }
...
<template>
  ...
  >
    ...
    <FormPageLoadingOverlay
      :loading="loading"
      :message="$t('pages.dropShippingOrderCreate.loading.detail')"
    >
      <div class="print-label-create__hero mb-6 d-flex align-center justify-space-between flex-wrap gap-3">
        <div>
          ...
          <h1 class="text-h4 font-weight-bold text-high-emphasis d-flex align-center gap-2">
            {{ $t('pages.dropShippingOrderDetail.title') }}
            <VChip
              size="small"
              color="primary"
              variant="tonal"
