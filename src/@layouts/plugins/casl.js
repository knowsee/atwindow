...
export const canNavigate = to => {
  ...
  // 未配置 ACL 的路由（多数 Dashboard）应对已登录用户放行，避免 ability.can(undefined, undefined) 误判
  const routesWithAcl = to.matched.filter(r => r.meta?.action && r.meta?.subject)
  if (!routesWithAcl.length)
    return true
